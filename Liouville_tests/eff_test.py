...
DOS_list = np.zeros((len(n_vec_measures), len(EF_list)))
N_list = np.zeros((len(n_vec_measures), len(EF_list)))
...
for i in range(len(t_vec)):
    ...
    if i == n_vec_measures[meas_count]:
        ...
        DOS_list[meas_count] = broad/np.pi*np.trace(delta_op, axis1=1, axis2=2)
        N_list[meas_count] = broad/np.pi*np.trace(delta_op@rho, axis1=1, axis2=2)
        meas_count += 1
...
for i in range(N_measures):
    ax.plot(EF_list, N_list[i,:], color=cmap(norm(t_vec_measures[i])), label=f't={round(t_vec_measures[i]/T, 3)}T')
#ax.legend()
...
fig, ax = plt.subplots()
ax.plot(t_vec_measures/T, N_list[ :,19], c='blue')
ax.plot(t_vec_measures/T, N_list[:,-20], c='red')
ax.set_xlabel('Tp')
ax.set_ylabel('DOS*n')
ax.set_xlim(0, 10)
# %%
