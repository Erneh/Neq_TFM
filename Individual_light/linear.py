...
folder_name = f'linear/N={N}_E={E}_Temp={Temp}_mu={mu:.2f}_G={gamma:.3f}/Nrand={N_random_vector}_nT={n_periods}_measT={meas_per_T}_stT={steps_per_T}'
...
for i in range(N_measures):
    ax.plot(n_mat_total[i,:,0], n_mat_total[i,:,1], color=cmap(norm(t_vec_measures[i])), label=f't={round(t_vec_measures[i]/T, 3)}T')
#ax.legend()
...
ax.vlines(0.0, 0, 1, color='grey', ls='--', alpha=0.8, zorder=1)
...
for (i, hE) in enumerate(hE_list):
    ind_time = np.where(n_mat_total[i,:,0] > hE)[0][0]
    n_timed = [n_mat_total[i,ind_time,1] for i in range(N_measures)]
